...
const scRunStoryPattern =
  /8\.\s+As a Pi user, I want `\/sc:run \[agent\] \[prompt\]`, so that I can run one Sandcastle-backed agent without invoking a backlog workflow\./;
const scRunAcceptanceCriteriaChecks = [
  {
    criterion: 'After reload, /sc:run is registered by the dev extension.',
    message: 'issue 00003 should require command registration',
  },
  {
    criterion: '/sc:run resolves the default agent when omitted and accepts free-form prompt text without treating words as flags unless explicitly declared.',
    message: 'issue 00003 should require deterministic prompt parsing',
  },
  {
    criterion: '/sc:run passes resolved Sandcastle options into the injected run capability without invoking real containers or agents in tests.',
    message: 'issue 00003 should require injected Sandcastle capability tests',
  },
  {
    criterion: '/sc:run writes a durable run record with status, agent, prompt summary, branch, commits, log path, and timestamps.',
    message: 'issue 00003 should require durable run records',
  },
  {
    criterion: '/sc:run reports the run id, final status, branch, commits, and log path to the user.',
...
defineIssueDocumentTest({
  commandSurface: '/sc:run',
  acceptanceCriteriaChecks: scRunAcceptanceCriteriaChecks,
  issueId: 'issue 00003',
  issueFileUrl,
  prdFileUrl,
  storyMessage: 'the parent PRD should contain the /sc:run user story that drives this work item',
