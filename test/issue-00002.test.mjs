...
const configAcceptanceCriteriaChecks = [
  {
    criterion: 'After reload, /backlog:config-raw is registered by the dev extension and /backlog:config-raw show displays effective repo config.',
    message: 'issue 00002 should require command registration and default show behavior',
  },
  {
    criterion: '/backlog:config-raw init creates missing .pi/sandcastle scaffold files and does not overwrite existing edited files without an explicit reset/force path.',
    message: 'issue 00002 should require init idempotency',
  },
  {
    criterion: '/backlog:config-raw get <path> returns one value, and missing/unsupported paths return clear user-facing errors.',
    message: 'issue 00002 should require get behavior and errors',
  },
  {
    criterion: '/backlog:config-raw set <path> <value> persists supported scalar values and leaves unrelated YAML content intact.',
    message: 'issue 00002 should require safe set behavior',
  },
  {
    criterion: '/backlog:config-raw reset restores supported paths to repo defaults without deleting unrelated project files.',
    message: 'issue 00002 should require bounded reset behavior',
  },
  {
    criterion: '/backlog:config-raw validate reports invalid agents, prompts, pipelines, models, sandbox providers, and missing runner/config files through the command response.',
    message: 'issue 00002 should require validation diagnostics',
  },
  {
    criterion: 'Tests instantiate the extension through a fake ExtensionAPI and fail if /backlog:config-raw is not registered.',
    message: 'issue 00002 should require fake ExtensionAPI registration tests',
  },
];

defineIssueDocumentTest({
  commandSurface: '/backlog:config-raw',
