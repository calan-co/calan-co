...
const backlogRunManagementAcceptanceCriteriaChecks = [
  {
    criterion: 'After reload, /work:runs, /work:status, and /work:resume are registered.',
    message: `${issueId} should require command registration`,
  },
  {
    criterion: '/work:runs lists backlog process runs and supports deterministic filtering by query text or item id.',
    message: `${issueId} should require filtered backlog run listing`,
  },
  {
    criterion: '/work:status infers active/latest backlog run when safe and reports ambiguity when multiple candidates exist.',
    message: `${issueId} should require safe status inference`,
  },
  {
    criterion: '/work:resume resumes only durable process runs with resumable provider/session metadata.',
...
const backlogRunManagementStories = [
  {
    ...
    pattern:
      /26\.\s+As a Pi user, I want `\/work:runs`, so that I can list durable backlog processing runs\./,
  },
  {
    storyName: 'backlog status user story',
    pattern:
      /27\.\s+As a Pi user, I want `\/work:status \[run-id\]`, so that I can inspect the current\/latest\/specified backlog processing run\./,
  },
  {
    storyName: 'backlog resume user story',
    pattern:
      /28\.\s+As a Pi user, I want `\/work:resume \[run-id\]`, so that I can continue the latest failed\/interrupted run by default and specify an ID only for disambiguation\./,
  },
];
