...
summary: Implement /work:process [query] --pipeline <pipeline>.
type: work-item
subtype: task
lifecycle: active
status: completed
...
- [x] Register /work:process in the dev extension.
...
- /work:process command.
- Durable backlog run records.
- Tests for parsing, inference, and multi-item processing.

## Acceptance Criteria

- [x] After reload, /work:process is registered.
- [x] /work:process review treats review as query text, not a pipeline name.
- [x] /work:process auth --pipeline implement uses auth as query and implement as pipeline.
