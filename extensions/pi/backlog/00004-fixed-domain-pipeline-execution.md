...
- [x] /work:pipeline command.
- [x] Pipeline run records with per-step details.
- [x] Tests for success, failure, and unknown pipeline cases.

## Acceptance Criteria

- [x] After reload, /work:pipeline is registered by the dev extension.
- [x] /work:pipeline validates the requested pipeline against repo config and rejects unknown names with available options.
