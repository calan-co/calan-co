---
id: wi-00003
title: Sandcastle API Adapter and /sc:run
summary: Create the direct Sandcastle API adapter and one-shot /sc:run command.
type: work-item
subtype: task
lifecycle: active
status: completed
...
- [x] Register /sc:run in extensions/pi-sandcastle/index.ts and parse optional agent plus free-form prompt text deterministically.
...
- [x] After reload, /sc:run is registered by the dev extension.
- [x] /sc:run resolves the default agent when omitted and accepts free-form prompt text without treating words as flags unless explicitly declared.
- [x] /sc:run passes resolved Sandcastle options into the injected run capability without invoking real containers or agents in tests.
- [x] /sc:run writes a durable run record with status, agent, prompt summary, branch, commits, log path, and timestamps.
- [x] /sc:run reports the run id, final status, branch, commits, and log path to the user.
