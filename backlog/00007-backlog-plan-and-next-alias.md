...
- [x] Register /backlog:plan and /backlog:next in the dev extension.
- [x] Build a planning capability over the read-only backlog capability from wi-00006.
- [x] Implement /backlog:plan [query] --iterations N to group ready work by dependencies, risk, and configured pipeline fit.
- [x] Implement /backlog:next [query] as the same planner with --iterations 1.
- [x] Return item groups, rationale, dependency notes, and recommended pipelines without durable selection state.
- [x] Add tests for query parsing, iteration limits, dependency grouping, /backlog:next aliasing, and read-only behavior.
...
- [x] After reload, /backlog:plan and /backlog:next are registered.
- [x] /backlog:plan accepts free-form query text and --iterations N controls the number of recommended processing iterations.
- [x] /backlog:next uses the same implementation as /backlog:plan --iterations 1.
