...
- [x] Register /work:list and /work:inspect in the dev extension.
- [x] Create a backlog capability that reads configured backlog sources through injected filesystem/path dependencies.
- [x] Implement /work:list [query] with deterministic filtering, sorting, and missing-source diagnostics.
- [x] Implement /work:inspect <item> to resolve by id, numeric id, title slug, or file path.
...
- [x] After reload, /work:list and /work:inspect are registered.
- [x] /work:list [query] returns matching backlog items in deterministic order and reports a clear missing-source error when no backlog source is configured.
- [x] /work:inspect <item> returns analysis, risks, relevant files, testing notes, and recommended pipeline for a resolvable item.
- [x] /work:inspect <item> reports a clear missing-item error when the target cannot be resolved.
